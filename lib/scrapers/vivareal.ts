...
const BASE_URL = "https://glue-api.vivareal.com/v2/listings";
const DOMAIN = "www.vivareal.com.br";
const USER_AGENT =
...
function normalizeListing(
  ...
  try {
    ...
    return {
      id: `vr-${id}`,
      source: "VivaReal",
      url: `https://www.vivareal.com.br/imovel/${id}`,
      title,
      transactionType,
    ...
    console.warn("[VivaReal] Failed to normalize listing:", err);
    return null;
  }
}

export async function scrapeVivaReal(config: {
  maxItems?: number;
  business?: string;
  ...
  console.log(`[VivaReal] Starting scrape (business=${business}, maxItems=${maxItems})...`);
  ...
  while (results.length < maxItems) {
    ...
    try {
      ...
      if (!listings || listings.length === 0) {
        console.log(`[VivaReal] No more listings at page ${page}`);
        break;
      }
      ...
      console.log(`[VivaReal] Page ${page}: ${listings.length} listings (total: ${results.length})`);

      if (listings.length < pageSize) break;
      page++;
    } catch (err) {
      console.warn(`[VivaReal] Failed at page ${page}:`, err);
      break;
    }
  }

  console.log(`[VivaReal] Scraped ${results.length} properties`);
  return results;
}
